...
const Sidebar = () => {
  ...
  return (
    <aside className="w-64 h-screen bg-content1 py-6 px-4 hidden md:block sticky top-0 border-r border-divider">
      <nav>
        {menu.map((m) => {
          const active = location.pathname === m.to; // adjust if you want startsWith for sections
          return (
            <Button
              key={m.to}
              as={Link}
              ...
              className={`justify-start mb-2 text-md w-full ${
                active ? "bg-primary text-white" : "bg-transparent text-foreground-500"
              }`}
            >
              {m.label}
            </Button>
